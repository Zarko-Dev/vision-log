...
export default function DashboardNavbar() {
    ...
    return (
        <>
            ...
            {/* Navigation - Dynamic Island Style (Always Floating) */}
            <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1 p-1.5 rounded-full border shadow-xl backdrop-blur-xl transition-all duration-300 bg-[#1C1C1E]/90 border-white/10">
                {navItems.map((item) => {
                    ...
                    return (
                        <Link 
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all ${
                                isActive 
                                    ? 'bg-[#6D28D9] text-white shadow-lg shadow-purple-900/50' 
